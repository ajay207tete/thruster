...
from tonclient.types import DeploySet, CallSet, Signer, ParamsOfEncodeMessage, ParamsOfSendMessage, ParamsOfWaitForTransaction, ParamsOfCompile, NetworkConfig
...
class TestShoppingContract(unittest.TestCase):
    ...
    def test_compile_contract(self):
        async def compile():
            with open("smart-contracts/contracts/ShoppingContract.fc", "r") as f:
                source = f.read()
            with open("smart-contracts/stdlib.fc", "r") as f:
                stdlib = f.read()
            compile_params = ParamsOfCompile(source=source, stdlib=stdlib)
            compiled = await self.client.abi.compile(params=compile_params)
    ...
    def test_deploy_contract(self):
        async def deploy():
            ...
            signer = Signer.Keys(keys=await self.client.crypto.generate_random_sign_keys())
