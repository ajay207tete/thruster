...
describe('Smart Contract Service Tests', () => {
  ...
  describe('Order Management Tests', () => {
    ...
    it('should handle getAllOrders errors', async () => {
      ...
      tonService.getAllOrders = jest.fn().mockRejectedValue(new Error('Database error'));

      const orders = await tonService.getAllOrders().catch(() => []);
      expect(orders).toEqual([]);

      // Restore original method
      tonService.getAllOrders = originalGetAllOrders;
    });
  });
  ...
  describe('Configuration Tests', () => {
    it('should validate contract configuration', () => {
      ...
      expect(validConfig.address).toMatch(/^0:[0-9A-Fa-f\-]+(-[0-9A-Fa-f\-]+)*$/);
