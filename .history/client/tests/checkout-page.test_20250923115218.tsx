...
const MockCheckoutScreen = () => {
  ...
  if (loading) {
    return React.createElement('div', null, 'Loading cart items...');
  }

  if (cartItems.length === 0) {
    return React.createElement('div', null,
      React.createElement('p', null, 'Your cart is empty.'),
      React.createElement('button', { onClick: () => mockRouter.back() }, 'Back to Cart')
    );
  }
    ...
    React.createElement('div', null,
      cartItems.map((item, index) =>
        React.createElement('div', { key: item._id },
          React.createElement('p', null, `${item.name} - $${item.price} x ${item.quantity}`)
        )
      )
    ),
    React.createElement('p', null, `Total: $${total.toFixed(2)}`),
    React.createElement('div', null,
      React.createElement('p', null, '💎 TON Smart Contract Payment'),
      React.createElement('p', null, `Contract: ${mockTonConfig.address}`),
      React.createElement('p', null, `Network: ${mockTonConfig.network}`)
    ),
...
describe('CheckoutScreen Component Tests', () => {
  ...
  it('should handle payment button press', async () => {
    ...
    expect(Alert.alert).toHaveBeenCalledWith(
      'Confirm Payment',
      expect.stringContaining('$449.96'),
      expect.any(Array)
    );
  });
