...
import { tonService, TonContractConfig } from '../services/tonService-updated';
...
describe('Shopping Flow Test Suite', () => {
  ...
  describe('Cart Management Tests', () => {
    it('should load cart items from AsyncStorage', async () => {
      ...
      expect(AsyncStorage.getItem).toHaveBeenCalledWith('cartItems');
    ...
    it('should save cart items to AsyncStorage', async () => {
      ...
      expect(AsyncStorage.setItem).toHaveBeenCalledWith('cartItems', JSON.stringify(updatedCart));
    });
