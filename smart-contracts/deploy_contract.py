...
class ContractDeployer:
    ...
    def _create_mock_compiled_data(self):
        ...
        # Mock ABI for the ShoppingContract - using proper format
        # For now, we'll use a dict and handle the conversion in the deployment
        self.contract_abi = {
            "version": "2.0",
            "functions": [
                {
    ...
    async def deploy_contract(self):
        ...
        # Convert dict ABI to proper format if needed
        abi_for_encoding = self.contract_abi
        if isinstance(self.contract_abi, dict):
            # Try to create a proper ABI object
            try:
                from tonclient.types import Abi
                abi_for_encoding = Abi(self.contract_abi)
            except Exception:
                # If that fails, try to use the dict directly
                abi_for_encoding = self.contract_abi

        params = ParamsOfEncodeMessage(
            abi=abi_for_encoding,
        ...
        return {
            ...
            "public_key": self.keypair["0QBjg8HT7GdRlO-4-7nC9ucEZ2XrcZS9xZ34TMU2DfodirJS"],
        }
